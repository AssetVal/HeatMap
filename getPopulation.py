...
def process_county_data_for_US():
...
def process_county_data_for_PR():
    # Load environment variables
    load_dotenv()
    api_key = os.getenv("CENSUS_KEY")
    ...
    # Use 2022 5-year estimates for PR
    year = "2022"
    # Puerto Rico's state FIPS code is 72
    url = f"https://api.census.gov/data/{year}/acs/acs5?get=NAME,B01003_001E&for=county:*&in=state:72&key={api_key}"
    
    response = requests.get(url)
    print(f"PR Status Code: {response.status_code}")
    print(f"PR Response Text: {response.text[:200]}...")
    
    if response.status_code != 200:
        raise Exception(f"Census API error for PR: {response.text}")
        
    try:
        census_data = response.json()
    except json.JSONDecodeError as e:
        print(f"JSON decode error for PR: {str(e)}")
        print(f"Full response: {response.text}")
        raise
    ...
    pr_features = [f for f in geojson['features'] if f['properties']['STATEFP'] == '72']
    ...
    print(f"Processed {len(pr_features)} Puerto Rico counties")
    print(f"Updated {output_path}")

if __name__ == "__main__":
    process_county_data_for_US()
    process_county_data_for_PR()
